...
// connected service and returns whatever succeeded. BRouter returns a mode 9
// GPX document (one <trk>, one <rte>) per alternative, carrying per-point way
// tags and, thanks to the injected "profile:showspeed" profile variable, a
// per-point <brouter:speed> that yields exact per-alternative route times.
// When BRouter is configured with acceptCompressedResult=true the payload is
// wrapped in an "ejY0" (base64 of "z64") + gzip envelope, which is decoded
// here before the GPX is handed to the router engine.
//
